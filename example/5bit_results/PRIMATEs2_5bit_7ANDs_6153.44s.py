# [2694881440, 969340170, 574899234, 806105100, 89169930, 605327624, 672662626]
S = []
for X in range(32):
	...
	g[5] = (x[4]^g[1]^g[2]) & (x[0]^x[1]^x[3]^g[0]^g[4]) 
	g[6] = (x[0]^x[2]^x[3]^g[1]^g[2]) & (x[1]^g[0]^g[4]) 

	y[0] = g[1] ^ 1 
	y[1] = x[0] ^ x[1] ^ x[3] ^ x[4] ^ g[0] ^ g[2] ^ g[3] ^ g[4] ^ g[5] ^ g[6] 
	y[2] = x[3] ^ x[4] ^ g[1] ^ g[5] ^ g[6] 
	y[3] = x[4] ^ g[0] ^ g[3] 
	y[4] = x[1] ^ x[2] ^ g[0] ^ g[4] ^ g[5] ^ g[6] 
